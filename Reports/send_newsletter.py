...
def main():
    ...
    print(f"\n=======================================================")
    print(f"  📑 Paperluz 電子報自動發送管線 (Issue {issue_num} ｜ {publish_date})")
    print(f"=======================================================\n")

    html_zh = build_newsletter_html_zh(issue_num, publish_date)
    html_en = build_newsletter_html_en(issue_num, publish_date)

    # 產製本地預覽檔
    preview_zh = os.path.join(SCRIPT_DIR, f"newsletter_preview_zh.html")
    preview_en = os.path.join(SCRIPT_DIR, f"newsletter_preview_en.html")
    ...
    if args.test_email:
        ...
        print(f"  📤 正在發送測試信至 {args.test_email} ({test_lang})...")
        try:
            send_email_smtp(args.test_email, subject, content, smtp_config)
            print(f"  ✓ 測試信發送成功！")
